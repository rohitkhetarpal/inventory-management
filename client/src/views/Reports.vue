    ...
    <div v-else>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="q in quarterlyData" :key="q.quarter">
                ...
                <td>{{ currencySymbol }}{{ q.total_revenue.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</td>
                <td>{{ currencySymbol }}{{ q.avg_order_value.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</td>
                <td>
            ...
            <div v-for="month in monthlyData" :key="month.month" class="bar-wrapper">
              <div class="bar-container">
                <div
                  ...
                  :title="currencySymbol + month.revenue.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US')"
                ></div>
              </div>
          ...
          <table class="reports-table">
            ...
            <tbody>
              <tr v-for="(month, index) in monthlyData" :key="month.month">
                ...
                <td>{{ currencySymbol }}{{ month.revenue.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US') }}</td>
                <td>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-value">{{ currencySymbol }}{{ totalRevenue.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
        </div>
        <div class="stat-card">
          <div class="stat-label">{{ t('reports.stats.avgMonthlyRevenue') }}</div>
          <div class="stat-value">{{ currencySymbol }}{{ avgMonthlyRevenue.toLocaleString(currentLocale === 'ja' ? 'ja-JP' : 'en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
        </div>
        <div class="stat-card">
...
export default {
  ...
  setup() {
    ...
    // Computed summary stats derived from data refs
    ...
    const formatMonth = (monthStr) => {
      // Convert YYYY-MM to locale-aware short month + year
    ...
    const getChangeValue = (current, previous) => {
      ...
      const locale = currentLocale.value === 'ja' ? 'ja-JP' : 'en-US'
      const absFormatted = Math.abs(change).toLocaleString(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })
      const symbol = currencySymbol.value
      if (change > 0) return `+${symbol}${absFormatted}`
      if (change < 0) return `-${symbol}${absFormatted}`
      return `${symbol}0.00`
    }
